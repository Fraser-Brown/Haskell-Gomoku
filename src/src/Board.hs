module Board where
    ...
    targetInARow :: [(Position, Col)] -> Int -> Int -> Col -> Maybe Col  
    targetInARow [] _ _ _ = Nothing
    
    targetInARow pieces target count col | target == count = Just(col)
                                         | snd (head pieces) == col = targetInARow (drop 1 pieces) target (count+1) col
                                         | otherwise = targetInARow (drop 1 pieces) target 1 (other(col))
    ...
    rowScanner pieces target | length pieces < target = Nothing
                             | otherwise = targetInARow sorted target 0 (snd (head sorted))
