...
findTimeLimt (x:inp) | take 2 x == "TL" = read(filter(\y -> y.isDigit) x) :: Int
...
findPieces :: [Strins] -> [(Position, Col)] -> [(Position, Col)]  --done this way to preserve move order, so that the undo will still work (potential for learning ai also)
findPieces [] r = r
...
findB (x:inp) | x == ',' = read(filter(\y -> y.isDigit) inp) :: Int
